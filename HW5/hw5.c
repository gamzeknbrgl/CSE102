...
/*In move function if a piece is black or white man, maximum capture compulsory works
 * but if a black or white man becomes a king, then it doesn't work with maximum capture compulsory
 * it only works with manuel directions */
int move(piece board[][8], int from_x, int from_y, int to_x, int to_y, player p) {
    int i, white_capture = 0, black_capture = 0, flag=1;
    if (p == white ) {
        ...
        if(board[from_x][from_y]==white_king && check_end_of_game(board)==-1 )
        {
        ...
        if(board[from_x][from_y]==black_man || board[from_x][from_y]==black_king)
            return -1;
        if(to_x<0 || to_y<0 || from_x<0 || from_y<0 || to_x>8 || to_y>8 || from_x>8 || from_y>8)
            return -2;
    ...
    if (p == black ) {
        ...
        if(board[from_x][from_y]==black_king && check_end_of_game(board)==-1 )
        {
